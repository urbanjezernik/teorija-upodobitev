...
\begin{zgled}
V okolju \GAP~se ni težko prepričati, da je najmanjša\footnote{Natančneje, obstajata dve taki neizomorfni grupi.} grupa $G$, v kateri $K(G)$ ne sovpada z izvedeno podgrupo $[G,G] = \langle K(G) \rangle$, moči $96$. Podamo jo lahko v njeni permutacijski obliki kot podgrupo $S_{12}$, generirano s permutacijama
\[
...
\begin{zgled}
Naj bo $G$ grupa moči $96$ iz zadnjega zgleda. Z algoritmom iz prejšnjega razdelka lahko hitro izračunamo njeno tabelo karakterjev. Grupa ima sicer $12$ razredov za konjugiranje, zato je njena tabela karakterjev kar velika. Iz nje lahko razberemo, da je element $(5 \ 6)(7 \ 8)$ v jedru vseh linearnih upodobitev, zato pripada izvedeni podgrupi $[G,G]$. Hkrati pa z uporabo Frobeniusove formule izračunamo, da ta element \emph{ne} pripada $K(G)$. Njegov konjugiranostni razred v $G$ sestoji iz treh elementov. Ko te elemente dodamo množici $K(G)$, dobimo ravno $[G,G]$.
\end{zgled}

\todo{Primer: komutatorji v $GL_2$ so ravno $SL_2$.}
