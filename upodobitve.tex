...
Opazujmo simetrično grupo $S_n$ na $n \in \NN$ nad poljem $\CC$. Ogledali smo si že tabele karakterjev za $n \leq 4$ in razložili, da je število nerazcepnih upodobitev enako številu konjugiranostnih razredov, to pa je enako številu razčlenitev $p(n)$. Družina simetričnih grup je posebna, saj zanjo presenetljivo obstaja eksplicitna korespondenca med konjugiranostnimi razredi in nerazcepnimi upodobitvami. Iz dane razčlenitve $(\lambda_1, \lambda_2, \dots, \lambda_k)$ števila $n$ lahko torej konstruiramo nerazcepno upodobitev grupe $S_n$ in za tem z nekoliko več truda določimo vrednosti karakterjev.

\subsubsection{Nerazcepne upodobitve}

\subsubsection{Vrednosti karakterjev}

Za družino simetričnih grup poznamo celo dokaj preprost način, kako lahko določimo vrednosti karakterjev nerazcepnih upodobitev. Dokaz je precej tehničen, temelji na teoriji simetričnih funkcij in nekoliko drugačnem\footnote{Standarden dokaz Frobeniusove formule uporablja jezik grupnih algeber in Youngovega simetrizatorja, glej {\literatura (Fulton in Harris 2004, [Section 4.1])}.} opisu nerazcepnih upodobitev $V_{\lambda}$, kot smo ga uporabili mi, zato ga brez prehude žalosti izpustimo.

\begin{izrek}[Frobeniusova formula]
Naj bo $\lambda = (\lambda_1, \lambda_2, \dots, \lambda_k)$ razčlenitev $n$ in $\chi_{\lambda}$ pripadajoči karakter. Naj bo $\conclass_{(i_1, i_2, \dots, i_n)}$ konjugiranostni razred. Tedaj je 
\[
    \chi_{\lambda}(\conclass_{(i_1, i_2, \dots, i_n)}) =
    \left[ \Delta(x) \cdot \prod_{j = 1}^n P_j(x)^{i_j} \right]_{(\ell_1, \ell_2, \dots, \ell_k)},
\]
kjer je
\[
...
\begin{zgled}
    \todo{Zgled: Vrednost karakterja v dolgem ciklu. Definiraj kljuke. Izpelji posledico o stopnjah. Zgled. Domača naloga: Izračunaj vrednost karakterja v transpoziciji.}
\end{zgled}
