...
\]
ki ji pravimo {\definicija Youngov simetrizator}. Vzdolž nje dobimo endospletično $\Fcal(\youngsym)$ regularne upodobitve, katere slika je vektorski prostor
\[
    V_{\lambda} = \image \Fcal(\youngsym) = \langle \youngsym * f \mid f \in \fun(S_n, \CC) \rangle.
\]
Na tem prostoru naravno deluje grupa $S_n$\footnote{Ker je $\Fcal(\youngsym)$ spletična, je to res invarianten podprostor. Ni pa težko videti, kako elementi grupe zares delujejo; za $g \in S_n$ element $\rho_{\fun}(g)$ preslika $\youngsym * f$ v $\youngsym * (\rho_{\fun}(g) \cdot f)$.}, dobljeno upodobitev označimo z $\rho_{\lambda}$. 
...
\begin{itemize}
    ...
    \[
        V_{\lambda} = \image \Fcal(\youngsym) \cong \frac{\fun_{S \backslash S_n}(S_n, \CC)}{\CC}.
    \]
...
Dokaz izreka bomo izpeljali s pomočjo naslednje leme, v kateri igra ključno vlogo delovanje Youngovega simetrizatorja $\widehat{\youngsym}(\rho_{\fun})$. V lemi uporabljamo leksikografsko delno urejenost $<$ na množici vseh razčlenitev. 
...
\todo{Definiraj splošne kljuke. Izpelji posledico o stopnjah.}
