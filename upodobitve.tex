...
\begin{zgled}
Naj bo $p = 2$. Grupa $G_2$ je moči $6$ in naravno deluje z matričnim množenjem na množici neničelnih vektorjev $\FF_2^2 \backslash \{ 0 \} = \{ e_1, e_2, e_1 + e_2 \}$. Na ta način dobimo homomorfizem
\[
    G_2 \to S_3, \quad
     \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix} \mapsto (1 \ 2), \ 
     \begin{pmatrix} 1 & 1 \\ 0 & 1 \end{pmatrix} \mapsto (2 \ 3),
    \]
ki je jasno surjektiven. Ker imata obe grupi enako moč, je celo izomorfizem, torej je $G_2 \cong S_3$.
\end{zgled}

Grupa $G_p$ ima še eno pomembno podgrupo, in sicer {\definicija specialno linearno grupo}
\[
    \SL_2(\FF_p) = \left\{ \begin{pmatrix} a & b \\ c & d \end{pmatrix} \mid a,b,c,d \in \FF_p, \ ad - bc = 1 \right\}
\]
obrnljivih matrik z determinanto $1$.
\todo{Generatorja te grupe, pojasni, razloži da je to izvedena podgrupa.}

\subsubsection{Konjugiranostni razredi}

V luči zadnjega zgleda brez škode predpostavimo, da je $p > 2$.

\subsubsection{Tabela karakterjev}

Grupa $\GL_2(\FF_p)$ naravno deluje na množici vektorjev v $\FF_p^2 \backslash \{ 0 \}$ z matričnim množenjem, kar lahko opišemo s homomorfizmom
\[
    \GL_2(\FF_p) \to S_{p^2 - 1}.
\]
