...
\[
    \EE_f(\rho) = \frac{1}{|G|} \sum_{g \in G} f(g) \rho(g) \in \hom(V,V).
\]

\begin{zgled}
Naj bo $\rho_{\hom}$ regularna upodobitev grupe $G$ in $f \in \hom(G,F)$ funkcija na $G$. Uteženo povprečje $\EE_f(\rho_{\hom})$ je linearni endomorfizem prostora $\hom(G,F)$. Pri tem se karakteristična funkcija identitete $1_1$ preslika v
\[
    \EE_f(\rho_{\hom}) 1_1 
    = \frac{1}{|G|} \sum_{g \in G} f(g) \rho_{\hom}(g) \cdot 1_1
    = \frac{1}{|G|} \sum_{g \in G} f(g) 1_{g^{-1}}.
\] 
Označimo invertiranje v grupi z $\inv \colon G \to G$, $g \mapsto g^{-1}$. Torej velja
\[
    \EE_f(\rho_{\hom}) 1_1
     = \frac{1}{|G|} \sum_{g \in G} (f \circ \inv)(g) 1_{g},
\]
s čimer smo funkcijo $\EE_f(\rho_{\hom}) 1_1$ identificirali kot $(f \circ \inv) / |G|$.
\end{zgled}
...
\[
    \displaystyle \EE_f(\pi) = 
    \frac{\langle f, \chi_{\pi} \circ \inv \rangle}{\deg(\pi)} \cdot \id_V,
\]
...
\[
    \EE_f(\pi) \cdot \pi(h) = \frac{1}{|G|} \sum_{g \in G} f(g) \pi(gh)
    = \frac{1}{|G|} \sum_{g \in G} f(g) \pi(h) \pi(h^{-1}gh).
\]
Izpostavimo $\pi(h)$ in na grupi $G$ uporabimo avtomorfizem $g \mapsto h g h^{-1}$, pa lahko zadnjo vsoto zapišemo kot
\[
    \pi(h) \frac{1}{|G|} \sum_{g \in G} f(hgh^{-1}) \pi(g).
\]
...
\[
    \tr \left( \EE_f(\pi) \right) = \frac{1}{|G|} \sum_{g \in G} f(g) \chi_{\pi}(g) = \langle f, \chi_{\pi} \circ \inv \rangle.
\]
Od tod izračunamo relevantni skalar kot $\langle f, \chi_{\pi} \circ \inv \rangle/\deg(\pi)$.
\end{dokaz}
