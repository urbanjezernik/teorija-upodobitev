...
V posebnem je vsaka endospletična regularne upodobitve enaka evalvaciji Fourierovi transformacije v neki fiksni funkciji. Nekoliko natančneje si poglejmo, kaj je ta evalvacija. Za funkciji $f,g \in \fun(G,F)$ je
\[
...
\[
    R(G) = \bigoplus_{\pi \in \Irr(G)} \ZZ \cdot \chi_{\pi} \subseteq \fun_{\cl}(G, F).
\]
...
\begin{zgled}
Naj bo $H$ ciklična grupa. Definirajmo funkcijo
\[
...
\[
    \Ind^H_K(c_K)(h) = \sum_{r \in R \colon h \in K} c_K(h) =
...
Naj bo $G$ grupa in $\rho$ njena končnorazsežna upodobitev nad algebraično zaprtim poljem karakteristike $0$. Tedaj je $\chi_{\rho}$ racionalna linearna kombinacija indukcij nerazcepnih karakterjev cikličnih podgrup grupe $G$.
\end{izrek}

Racionalnim kombinacijam se lahko izognemo, če razširimo razred podgrup s cikličnih na {\definicija $p$-elementarne podgrupe} $E_p$ grupe $G$. To so podgrupe, ki so izomorfne direktnemu produktu ciklične grupe in $p$-grupe.

\begin{izrek}[Brauerjev izrek]
Naj bo $G$ grupa in $F$ algebraično zaprto polje karakteristike $0$. Tedaj je
\[
    R(G) = \sum_{H \in \bigcup_{p \in \PP} E_p} \Ind(R(H)).
\]
\end{izrek}
...
Naj bo $\lambda = (\lambda_1, \lambda_2, \dots, \lambda_k)$ razčlenitev $n$. Nerazcepno upodobitev grupe $S_n$, prirejeno $\lambda$, kot ponavadi iščemo z indukcijo iz podgrup. Razčlenitev $\lambda$ lahko interpretiramo kot ciklični tip permutacij, zato se naravno ponuja {\definicija Youngova podgrupa}
\[
    P = S_{\lambda_1} \times S_{\lambda_2} \times \cdots \times S_{\lambda_k} \leq S_n.
\]
...
\begin{zgled} \leavevmode
\begin{itemize}
    \item Naj bo $\lambda = (n)$. Tedaj je $P = S_n$ in $Q = 1$, zato je $\youngsym_{\lambda} = 1$. Za funkcijo $f \in \fun(S_n, \CC)$ je $\Fcal(1) \cdot f = 1 * f = |G| \cdot \EE(f)$ in grupa $S_n$ deluje trivialno na tej funkciji. S tem je
    \[
        V_{\lambda} = \image \Fcal(1) = \CC
    \]
    in dobimo trivialno upodobitev.

    \item Naj bo $\lambda = (1,1,\dots,1)$. Tedaj je $P = 1$ in $Q = S_n$, zato je $\youngsym_{\lambda} = \sgn$. Za funkcijo $f \in \fun(S_n, \CC)$ je
    \[
    ...
    \[
        \fun_{S \backslash S_n}(S_n, \CC) \to \fun_{S \backslash S_n}(S_n, \CC), \quad
    ...
    \[
        V_{\lambda} = \image \Fcal(\youngsym_{\lambda}) \cong \frac{\fun_{S \backslash S_n}(S_n, \CC)}{\CC}.
    \]
...
Za družino simetričnih grup poznamo celo dokaj preprost način, kako lahko določimo vrednosti karakterjev nerazcepnih upodobitev. Izrekli ga bomo v jeziku polinomskega kolobarja $\CC[\mathbf{x}] = \CC[x_1, x_2, \dots, x_k]$. Potrebovali bomo nekaj posebnih polinomov iz tega kolobarja, in sicer {\definicija diskriminanto}
\[
...
Dokaz temelji na poznavanju osnov teorije simetričnih funkcij.\footnote{Te osnove študent-ka ponavadi spozna pri predmetu Kombinatorika.}. Poleg tega dokaz sloni na nekoliko drugačnem\footnote{Standarden dokaz Frobeniusove formule uporablja jezik grupnih algeber in Youngovega simetrizatorja, glej {\literatura (Fulton in Harris 2004, [Section 4.1])}.} opisu nerazcepnih upodobitev $V_{\lambda}$, kot smo ga uporabili mi, zato ga brez prehude žalosti izpustimo. Poglejmo pa si nekaj primerov uporabe izreka.
...
\todo{Definiraj splošne kljuke. Izpelji posledico o stopnjah. Zgled. Domača naloga: Izračunaj vrednost karakterja v transpoziciji.}

S Frobeniusovo formulo lahko določimo stopnje nerazcepnih upodobitev simetrične grupe.
...
Ne spreglejmo, da iz Frobeniusove formule neposredno sledi, da so vrednosti karakterjev zelo posebne.\footnote{To posledico bi sicer lahko izpeljali tudi brez zahtevne Frobeniusove formule, glej na primer {\literatura (Serre 1977, [Section 13.1, Corollary 1])}.}

\begin{posledica}
Vrednosti nerazcepnih karakterjev simetrične grupe so cela števila.
\end{posledica}

V zvezi s tabelo karakterjev simetrične grupe omenimo le še sodobnejši presenetljiv rezultat \href{https://link.springer.com/article/10.1007/s00209-014-1290-x}{(Miller 2014)}, v katerem avtor dokaže, da so vrednosti skoraj vseh karakterjev v skoraj vseh grupnih elementih ničelne. Natančneje, če enakomerno naključno izberemo $g \in S_n$ in $\pi \in \Irr(S_n)$, potem je
\[
...
\todo[inline]{Omeni Kazhdan-Lusztig teorijo, generične tabele karakterjev (Malle), CFSG in ATLAS}
...
\todo{Primer: komutatorji v $GL_2$ so ravno $SL_2$, razberemo iz tabele karakterjev $GL_2$.}
