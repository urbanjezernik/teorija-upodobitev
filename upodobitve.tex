...
Zopet bomo za dokaz uporabili metodo povprečenja po grupi, a bomo to povprečenje še \emph{utežili}. Za dano upodobitev $\rho$ grupe $G$ na prostoru $V$ in poljubno funkcijo $f \in \hom(G,F)$ definiramo {\definicija uteženo povprečje}
\[
    \EE_f(\rho) = \sum_{g \in G} f(g) \rho(g) \in \hom(V,V).
\]
Kadar je $F = \CC$, uteženemu povprečenju rečemo tudi {\definicija nekomutativna Fourierova transformacija}.
...
\begin{trditev}
Naj bo $G$ končna grupa. Za vsako nerazcepno kompleksno upodobitev $\pi$ in vsak $g \in G$ je $|\chi_{\pi}(g)| \leq \deg(\pi)$ in $\chi_{\pi}(g) \in \bar\ZZ$.
\end{trditev}
\begin{dokaz}
...
\[
    \chi_{\pi}(g) = \sum_{\lambda \in \Eig(\pi)} \lambda \in \bar\ZZ
    \quad \text{in} \quad
    |\chi_{\pi}(g)| \leq \sum_{\lambda \in \Eig(\pi)} |\lambda| = \deg(\pi).
\]
\end{dokaz}
...
\begin{izrek}[o stopnjah upodobitev]
    Naj bo $G$ končna grupa. Stopnja vsake nerazcepne kompleksne upodobitve $\pi$ deli $|G|$.
    \end{izrek}
...
Dokaz bomo navezali na edino mesto, kjer smo že videli ulomek $|G|/\deg(\pi)$, in sicer je to lema o uteženem povprečju. Ko funkcija, vzdolž katere izvedemo povprečje, slika v kolobar algebraičnih celih števil, lahko lemo o uteženem povprečju zaostrimo na naslednji način.
    ...
    \]
    Preveriti moramo torej, da je to algebraično celo število. Funkcijo $f$ lahko razvijemo kot vsoto karakterističnih funkcij konjugiranostnih razredov s koeficienti v $\bar\ZZ$. Ker $\bar\ZZ$ tvori kolobar, bo torej trditev dovolj preveriti za primer, ko je $f = 1_{\calC}$ za nek konjugiranostni razred $\calC$ v $G$.
    
    Vse narazcepne upodobitve lahko obravnavamo v enem zamahu, in sicer tako, da opazujemo regularno upodobitev in pripadajoče uteženo povprečje $\EE_{1_{\calC}}(\rho_{\hom})$. Na vsaki od podupodobitev, ki je izomorfna $\pi$, to uteženo povprečje deluje kot $\EE_{1_{\calC}}(\pi)$, torej kot skalarno množenje z gornjim številom. To število je zato lastna vrednost preslikave $\EE_{1_{\calC}}(\rho_{\hom})$.
    
    Vemo že, da $\EE_{1_{\calC}}(\rho_{\hom})$ deluje na naravni bazi iz karakterističnih funkcij $1_x$ za $x \in G$ kot
    \[
        \EE_{1_{\calC}}(\rho_{\hom}) \cdot 1_x = (\rho_{\hom}(x) \cdot 1_{\calC}) \circ \inv \in \hom(G, \{ 0, 1 \}).
    \]
    V tej bazi ima torej $\EE_{1_{\calC}}(\rho_{\hom})$ matriko s koeficienti v množici $\{ 0, 1 \}$. Karakteristični polinom te matrike ima zato celoštevilske koeficiente, torej so lastne vrednosti preslikave $\EE_{1_{\calC}}(\rho_{\hom})$ algebraična cela števila.
    \end{dokaz}
...
\todo[inline]{
    ...
    - konkreten zgled, pokaži, kaj vse se da prebrati o grupi iz tega zgleda, morda $\GL_2(\field_2)$ in $S_5$
