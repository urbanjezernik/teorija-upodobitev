...
\[
    ...
    = \sum_{h \in G_p} p_{S}(h,g) \cdot \PP(X_{n-1} = h).
\]
...
\todo{diagonalizacija $M$}

\subsection{Naključno množenje podobnih matrik}

Namesto abstraktne teorije bomo slučajne sprehode raziskali kar na konkretnem primeru, ki je dovolj reprezentativen za splošen primer. Obravnavajmo grupo $G_p = \GL_2(\FF_p)$ za $p > 3$, ki smo jo že dodobra spoznali. V njej za generirajočo množico $S$ izberimo konjugiranostni razred regularnih polenostavnih elementov, ki so podobni matriki
\[
    A = \begin{pmatrix}
        \delta & 0 \\ 0 & 1 
    \end{pmatrix},
\] 
kjer je $\delta$ generator ciklične grupe obrnljivih elementov končnega polja $\FF_p^*$. Množica $S$, ki jo bomo tukaj označevali z bolj standardno oznako $\conclass$, sestoji torej iz vseh matrik, ki so v $G_p$ podobne $A$.
