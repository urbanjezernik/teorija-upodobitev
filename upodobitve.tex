...
Zopet bomo za dokaz uporabili metodo povprečenja po grupi, a bomo to povprečenje še \emph{utežili}. Za dano upodobitev $\rho$ grupe $G$ na prostoru $V$ in poljubno funkcijo $f \in \hom(G,F)$ definiramo {\definicija uteženo povprečje}\footnote{Kadar je $F = \CC$, uteženemu povprečenju rečemo tudi {\definicija nekomutativna Fourierova transformacija}. Njeno povezavo z običajno teorijo Fourierove transformacije bomo spoznali nekoliko kasneje.}
\[
    \EE_f(\rho) = \sum_{g \in G} f(g) \rho(g) \in \hom(V,V).
\]

\begin{zgled}
Naj bo $\rho_{\hom}$ regularna upodobitev grupe $G$ in $f \in \hom(G,F)$ funkcija na $G$. Uteženo povprečje $\EE_f(\rho_{\hom})$ je linearni endomorfizem prostora $\hom(G,F)$. Pri tem se karakteristična funkcija $1_x$ za $x \in G$ preslika v
\[
...
Kadar ne poznamo vseh nerazcepnih karakterjev, nam naslednji preprost kriterij lahko pomaga najti nove.
...
\todo[inline]{
    - večkratnosti 4.3.5 (nerazcepne podupodobitve)
    - nerazcepnost
    - zgled: $S_4$ tabela karakterjev iz Fulton-Harris
    }
