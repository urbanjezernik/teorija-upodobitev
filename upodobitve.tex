...
Vse podobno velja, kadar imamo namesto ene same nerazcepne upodobitve \emph{končno mnogo} nerazcepnih upodobitev $\{ \pi_i \}_{i \in I}$ dane grupe $G$. Vsaka od njih nam podari svoje matrične koeficiente $\MK(\pi_i)$. Ti so enakim izotipičnim komponentam v regularni upodobitvi in te komponente tvorijo notranjo direktno vsoto. Matrični koeficienti vseh teh upodobitev so torej linearno neodvisni med sabo. Vseh skupaj je $\sum_{i \in I} \deg(\pi_i)^2$. Vsi ti koeficienti so pravzaprav elementi prostora funkcij $\hom(G,F)$, zato lahko v primeru, ko je grupa končna, od tod izpeljemo neenakost
\[
