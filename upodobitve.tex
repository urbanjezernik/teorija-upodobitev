...
\item {\literatura J. P. Serre, {\em Linear Representation of Finite Groups}, Springer GTM 42, 1977.}
\end{itemize}
...
Zopet bomo za dokaz uporabili metodo povprečenja po grupi, a bomo to povprečenje še \emph{utežili}. Za dano funkcijo $f \in \hom(G,F)$ in poljubno upodobitev $\rho$ grupe $G$ na prostoru $V$ definiramo {\definicija uteženo povprečje}
\[
    \EE_f(\rho) = \sum_{g \in G} f(g) \rho(g) \in \hom(V,V).
\]
Kadar je $F = \CC$, uteženemu povprečenju rečemo tudi {\definicija nekomutativna Fourierova transformacija} in uporabljamo oznako $\hat{f}(\rho)$.
...
\begin{itemize}
    ...
    \[
        \EE_f(\chi_j) = \sum_{x \in \ZZ/n\ZZ} f(x) e^{2 \pi i j x / n }.
    \]
    Vektorju $(f(1), f(2), \dots, f(n)) \in \CC^n$ na ta način priredimo vektor $(\EE_f(\chi_1), \EE_f(\chi_2), \dots, \EE_f(\chi_n)) \in \CC^n$. To prirejanje je v numerični matematiki znano pod imenom {\definicija diskretna Fourierova transformacija} in je fundamentalno v digitalnem procesiranju signalov.

    \item Naj bo $\rho_{\hom}$ regularna upodobitev grupe $G$ in $f \in \hom(G,F)$ funkcija na $G$. Uteženo povprečje $\EE_f(\rho_{\hom})$ je linearni endomorfizem prostora $\hom(G,F)$. Pri tem se karakteristična funkcija $1_x$ za $x \in G$ preslika v
\[
    \EE_f(\rho_{\hom}) \cdot 1_x 
    = \sum_{g \in G} f(g) \rho_{\hom}(g) \cdot 1_x
    = \sum_{g \in G} f(g) 1_{xg^{-1}}
    = \sum_{g \in G} f(g^{-1}x) 1_{g}.
\] 
Označimo invertiranje v grupi z $\inv \colon G \to G$, $g \mapsto g^{-1}$. Torej velja
\[
    \EE_f(\rho_{\hom}) \cdot 1_x
     = \sum_{g \in G} ((\rho_{\hom}(x) \cdot f) \circ \inv)(g) 1_{g}
     = (\rho_{\hom}(x) \cdot f) \circ \inv.
\]
V posebnem pri $x = 1$ dobimo $\EE_f(\rho_{\hom}) \cdot 1_1 = f \circ \inv$. 
\end{itemize}
\end{zgled}

\begin{lema}[o uteženem povprečju]
Naj bo $G$ končna grupa in $F$ algebraično zaprto polje karakteristike tuje $|G|$. Za vsako nerazcepno upodobitev $\pi$ na prostoru $V$ in razredno funkcijo $f$ je
\[
    \EE_f(\pi) = 
    \frac{|G| \cdot \langle f, \chi_{\pi} \circ \inv \rangle}{\deg(\pi)} \cdot {\textstyle \id_V},
\]
kjer smo označili $\inv \colon G \to G$, $g \mapsto g^{-1}$.
\end{lema}
\begin{dokaz}
Za vsak $h \in G$ velja
\[
    \EE_f(\pi) \cdot \pi(h) =  \sum_{g \in G} f(g) \pi(gh)
    =  \sum_{g \in G} f(g) \pi(h) \pi(h^{-1}gh).
\]
Izpostavimo $\pi(h)$ in na grupi $G$ uporabimo avtomorfizem $g \mapsto h g h^{-1}$, pa lahko zadnjo vsoto zapišemo kot
\[
    \pi(h)  \sum_{g \in G} f(hgh^{-1}) \pi(g).
\]
Ker je $f$ razredna funkcija, je dobljeno ravno enako $\pi(h) \cdot \EE_f(\pi)$. Uteženo povprečje je torej v tem primeru spletična v $\hom_G(\pi, \pi)$. Po Schurovi lemi sklepamo, da je $\EE_f(\pi)$ skalarni večkratnik identitete, iz leme o uteženem povprečju pa je njegova sled enaka
\[
    \tr \left( \EE_f(\pi) \right) =  \sum_{g \in G} f(g) \chi_{\pi}(g) = |G| \cdot \langle f, \chi_{\pi} \circ \inv \rangle.
\]
Od tod izračunamo relevantni skalar kot $|G| \cdot \langle f, \chi_{\pi} \circ \inv \rangle/\deg(\pi)$.
\end{dokaz}

Uteženo opremljeni lahko z lahkoto izpeljemo izrek.

\begin{dokaz}[Dokaz izreka o bazi razrednih funkcij]
Predpostavimo, da nerazcepni karakterji \emph{ne} razpenjajo prostora razrednih funkcij. Torej obstaja funkcija $f \in \hom_{\cl}(G,F)$, ki je vsebovana v ortogonalnem komplementu vseh nerazcepnih karakterjev. Za vsak $\pi \in \Irr(G)$ velja torej $\langle f, \chi_{\pi} \rangle = 0$, kar lahko zapišemo tudi kot $\langle f \circ \inv, \chi_{\pi} \circ \inv \rangle = 0$. Uteženo povprečje vzdolž razredne funkcije $f \circ \inv$ je po lemi zato trivialno. Ker to velja za vsako nerazcepno upodobitev, mora veljati tudi za regularno upodobitev, ki je vsota nerazcepnih upodobitev, se pravi $\EE_{f \circ \inv}(\rho_{\hom}) = 0$. Po zadnjem zgledu to implicira $(f \circ \inv) \circ \inv = 0$, se pravi $f = 0$. 
\end{dokaz}
...
Dokaz bomo navezali na edino mesto, kjer smo že videli ulomek $|G|/\deg(\pi)$, in sicer je to lema o uteženem povprečju. Ko funkcija, vzdolž katere izvedemo povprečje, slika v kolobar algebraičnih celih števil, lahko lemo o uteženem povprečju zaostrimo na naslednji način.

    \begin{lema}
    Naj bo $G$ končna grupa. Za vsako nerazcepno upodobitev $\pi$ nad $\CC$ in funkcijo $f \in \hom_{\cl}(G, \bar{\ZZ})$ je uteženo povprečje $\EE_f(\pi)$ skalarno množenje z algebraičnim celim številom.
    \end{lema}
    \begin{dokaz}
    Vemo že, da je uteženo povprečje $\EE_f(\pi)$ skalarno množenje s številom
    \[
        \frac{|G| \cdot \langle f, \chi_{\pi} \circ \inv \rangle}{\deg(\pi)}.
    \]
    ...
    Vse narazcepne upodobitve lahko obravnavamo v enem zamahu, in sicer tako, da opazujemo regularno upodobitev in pripadajoče uteženo povprečje $\EE_{1_{\conclass}}(\rho_{\hom})$. Na vsaki od podupodobitev, ki je izomorfna $\pi$, to uteženo povprečje deluje kot $\EE_{1_{\conclass}}(\pi)$, torej kot skalarno množenje z gornjim številom. To število je zato lastna vrednost preslikave $\EE_{1_{\conclass}}(\rho_{\hom})$.
    
    Vemo že, da $\EE_{1_{\conclass}}(\rho_{\hom})$ deluje na naravni bazi iz karakterističnih funkcij $1_x$ za $x \in G$ kot
    \[
        \EE_{1_{\conclass}}(\rho_{\hom}) \cdot 1_x = (\rho_{\hom}(x) \cdot 1_{\conclass}) \circ \inv \in \hom(G, \{ 0, 1 \}).
    \]
    V tej bazi ima torej $\EE_{1_{\conclass}}(\rho_{\hom})$ matriko s koeficienti v množici $\{ 0, 1 \}$. Karakteristični polinom te matrike ima zato celoštevilske koeficiente, torej so lastne vrednosti preslikave $\EE_{1_{\conclass}}(\rho_{\hom})$ algebraična cela števila.
    \end{dokaz}
    
    \begin{dokaz}[Dokaz izreka o stopnjah upodobitev]
    Naj bo $\pi \in \Irr(G)$. Uporabimo lemo s funkcijo $f = \chi_{\pi} \circ \inv$ in zaključimo
    \[
        \frac{|G| \cdot \langle \chi_{\pi}, \chi_{\pi} \rangle}{\deg(\pi)} = \frac{|G|}{\deg(\pi)} \in \bar{\ZZ} \cap \QQ = \ZZ.
    \]
    \end{dokaz}
...
\begin{zgled}
Opazujmo grupo $S_4$ nad poljem $\CC$. Vemo že, da je število nerazcepnih upodobitev enako $5$. Določimo jih. \todo{}
\end{zgled}
...
Take projekcije ni težko najti za $\11$-izotipično komponento. Naj bo $G$ končna grupa z upodobitvijo $\rho$ na prostoru $V$ nad ugodnim poljem. Povprečna vrednost upodobitve $\EE(\rho)$ je projekcijska spletična v $\hom_G(V,V)$ na podprostor fiksnih vektorjev $V^G$, se pravi na $\11$-izotipično komponento v $V$. Za splošnejše izotipične komponente uporabimo utežena povprečja.
...
\[
    \frac{\deg(\pi)}{|G|} \cdot \EE_{\chi_{\pi} \circ \inv}(\rho)
\]
projektor na $\pi$-izotipično komponento v $\rho$.
\end{trditev}
\begin{dokaz}
Iz leme o uteženem povprečju izpeljemo, da za vsaki nerazcepni upodobitvi $\pi_1$, $\pi_2$ na prostorih $V_1$, $V_2$ velja
\[
    \frac{\deg(\pi_1)}{|G|} \cdot \EE_{\chi_{\pi_1} \circ \inv}(\pi_2) = \begin{cases}
        {\textstyle \id_{V_2}} & \pi_1 \cong \pi_2, \\
        0 & \text{sicer.}
    \end{cases}
\]
Ko upodobitev $\rho$ razstavimo na direktno vsoto nerazcepnih podupodobitev, je linearni endomorfizem $\deg(\pi)/|G| \cdot \EE_{\chi_{\pi} \circ \inv}(\rho)$ torej ničeln na podupodobitvah, ki niso izomorfne $\pi$, in identiteta na podupodobitvah, ki so izomorfne $\pi$. Ta endomorfizem je torej projektor na direktno vsoto podupodobitev, ki so izomorfne $\pi$, torej ravno na $\pi$-izotipično komponento.
\end{dokaz}

\begin{zgled}
Naj bo $\rho_{\hom}$ regularna upodobitev grupe $G$. Vemo že, da za vsako funkcijo $f \in \hom(G,F)$ velja $\EE_f(\rho_{\hom}) \cdot 1_1 = f \circ \inv$. Torej je projekcija funkcije $1_1$ na $\pi$-izotipično komponento enaka
\[
    \frac{\deg(\pi)}{|G|} \cdot \EE_{\chi_{\pi} \circ \inv}(\rho_{\hom}) \cdot 1_1 = 
...
Naj bosta $f_1, f_2 \in \hom(G,F)$ funkciji in $\rho$ upodobitev grupe $G$. Kompozicija uteženih povprečij $\EE_{f_1}(\rho) \cdot \EE_{f_2}(\rho)$ je enaka
\[
    \sum_{g_1, g_2 \in G} f_1(g_1) f_2(g_2) \rho(g_1 g_2) =
    \sum_{g \in G} \left( \sum_{g_1, g_2 \in G \colon g_1 g_2 = g} f_1(g_1) f_2(g_2) \right) \rho(g).
\]
Člen v notranji vsoti je {\definicija konvolucija} funkcij $f_1$ in $f_2$, se pravi
\[
    (f_1 * f_2)(g) = \sum_{g_1, g_2 \in G \colon g_1 g_2 = g} f_1(g_1) f_2(g_2).
\]
Torej velja
\[
    \EE_{f_1}(\rho) \cdot \EE_{f_2}(\rho) =
    \EE_{f_1 * f_2}(\rho)
\]
in uteženo povprečje pretvarja konvolucijo funkcij v produkt linearnih preslikav.

Četrti princip harmonične analize je, da iz Fourierove transformacije izvlečemo podatke o začetni funkciji.
...
\todo[inline]{
    ...
    - konkreten zgled, pokaži, kaj vse se da prebrati o grupi iz tega zgleda, morda $\GL_2(\FF_2)$ in $S_5$
    - upodobitve v potencah: vključi kakšen preprost zgled; omeni, da gre za to, da ni treba razstavljati regularne upodobitve; ta zgled lahko potem kasneje razširimo z \GAP.
}
