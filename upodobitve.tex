...
\newenvironment{trditevbrezimena}
    {\begin{center}\begin{tabular}{|p{0.7\textwidth}|}
    \hline
    }
    { 
    \\\hline
...
\[
    \rho^\sigma \colon G \to \GL_n(F), \quad
...
\begin{domacanaloga}
    Naj bosta $G,H$ grupi s homomorfizmom $\phi \colon H \to G$. Za vsako upodobitev $\rho$ v $\Rep_G$ in upodobitev $\sigma$ v $\Rep_H$ velja
    \[
        \Ind^G_H(\Res^G_H(\rho) \otimes \sigma) \cong
...
\subsection{Nerazcepnost in polenostavnost}

Pogosto nas bo zanimalo, če lahko dano upodobitev $\rho$ grupe $G$ na prostoru $V$ zapišemo kot direktno vsoto nekih podupodobitev in na ta način upodobitev $\rho$ \emph{razstavimo} na preprostejše upodobitve, podobno kot razstavimo števila na praštevila. To vprašanje se naravno pojavi, kadar ima dana upodobitev $\rho$ neko podupodobitev $\tilde\rho$ na podprostoru $W \leq V$. Seveda lahko vselej najdemo vektorski prostor $U \leq V$, za katerega je $V = U \oplus W$, vsekakor pa ni jasno, če lahko najdemo tak podprostor $U$, ki je celo $G$-invarianten. V tem primeru rečemo, da smo našli {\definicija komplementirano podupodobitev} podupodobitve $\tilde\rho$.\footnote{Če komplementirana podupodobitev obstaja, potem je enolično določena (do izomorfizma upodobitev), saj je izomorfna kvocientu $\rho/\tilde\rho$.}

\begin{zgled}
\begin{itemize}
    \item Opazujmo permutacijsko upodobitev simetrične grupe $S_3$ na prostoru $\RR[\{ 1,2,3 \}] = \RR^3$. Delovanje grupe $S_3$ ohranja vektor $e_1 + e_2 + e_3$, zato ima ta upodobitev trivialno enorazsežno podupodobitev, dano s podprostorom $\langle e_1 + e_2 + e_3 \rangle$. Eden od komplementov tega podprostora je $\langle e_1 - e_2, e_2 - e_3 \rangle$, ki je hkrati $S_3$-invariaten podprostor.\footnote{Velja na primer $(1 \ 3 \ 2) \cdot (e_1 - e_2) = e_3 - e_1 = -(e_1 - e_2) - (e_2 - e_3)$.} Permutacijska upodobitev $S_3$ je zato direktna vsota enorazsežne in dvorazsežne podupodobitve.

    \item Naj grupa $\RR$ deluje na realnem prostoru $\RR^2$ s homomorfizmom
    \[
        \rho \colon \RR \to \GL_2(\RR), \quad
        x \mapsto 
        \begin{pmatrix} 
        1 & x \\ 0 & 1 
    \end{pmatrix}.
...
Nima torej vsaka podupodobitev komplementirane podupodobitve. Ni težko preveriti, da obstoj takih komplementov vselej izhaja iz projekcijskih\footnote{Linearna preslikava $A \colon X \to X$ je projekcija na podprostor $Y \leq X$, če je $A^2 = A$ in $\image A = Y$.} spletičen.
...
Vrnimo se k začetni ideji o \emph{razstavljanju} dane upodobitve. Naj bo $G$ grupa z upodobitvijo $\rho$ na prostoru $V \neq 0$. Kadar \emph{ne} obstaja noben $G$-invarianten podprostor prostora $V$ (razen prostorov $0$ in $V$), tedaj rečemo, da je upodobitev $\rho$ {\definicija nerazcepna}.\footnote{Rečemo tudi, da je $V$ {\definicija enostavna} upodobitev.} V tem primeru upodobitve seveda ne moremo razstaviti na enostavnejše.
...
\begin{lema}{\color{modra} (Schurova lema)}
...
Kadar lahko dano upodobitev $\rho$ zapišemo kot direktno vsoto nerazcepnih upodobitev $\bigoplus_{i \in I} \rho_i$, tedaj rečemo, da je $\rho$ {\definicija polenostavna} upodobitev. Če so pri tem vse podupodobitve $\rho_i$ izomorfne med sabo, tedaj rečemo, da je $\rho$ {\definicija izotipična} upodobitev.

\begin{zgled}
Opazumo permutacijsko upodobitev grupe $S_3$ na prostoru $\RR^3$. Ta upodobitev je direktna vsote trivialne enorazsežne upodobitve na prostoru $\langle e_1 + e_2 + e_3 \rangle$ in dvorazsežne upodobitve na prostoru $\langle e_1 - e_2, e_2 - e_3 \rangle$. Če označimo $u_1 = e_1 - e_2$ in $u_2 = e_2 - e_3$, lahko slednjo upodobitev opišemo s homomorfizmom
\[
    \rho \colon S_3 \to \GL(\langle u_1, u_2 \rangle), \quad
    (1 \ 2) \mapsto \begin{pmatrix} 
        -1 & 1 \\ 0 & 1 
    ...
    \end{pmatrix}.
\]
Lastna vektorja $\rho((1 \ 2))$ sta $u_1$ in $u_1 + 2 u_2$. Noben od teh dveh vektorjev ni hkrati lastni vektor $\rho((1 \ 2 \ 3))$. Torej je upodobitev $\rho$ stopnje $2$, hkrati pa nima enorazsežnih podupodobitev, torej je nerazcepna. Permutacijsko upodobitev $S_3$ na $\RR^3$ smo s tem zapisali kot direktno vsoto nerazcepnih upodobitev, torej je ta upodobitev polenostavna. Ni pa izotipična, saj sta nerazcepna sumanda med sabo seveda neizomorfna.
\end{zgled}

V predzadnjem zgledu smo videli, da \emph{niso} vse upodobitve polenostavne in da torej ne moremo razstaviti vseh upodobitev na nerazcepne.\footnote{V nadaljevanju bomo pokazali, da so upodobitve končnih grup nad poljem kompleksnih števil vselej poenostavne.} Polenostavnost dane upodobitve je tesno povezana z obstojem komplementiranih podupodobitev.
...
\subsection{Dekompozicijska vrsta}

- Theorem 2.7.1 (Jordan-Holder-Noether theorem)

\subsection{Spletične med nerazcepnimi upodobitvami}

-  (Schurs Lemma, version II). 
- Corollary 2.7.17 (Abelian groups and central character).

\subsection{Večkratnosti in izotipične komponente}

- multiplicities and isotypical components
...
\todo{$SU_2$ bi lahko pokazali že prej, kot zgled upodobitev neskončne grupe. kako težko je dokazati nerazcepnost?}

- topološke grupe (zvezne upodobitve)
- unitarne na hilbertovih prostorih (končne smo že, $SU_2$)
- upodobitve $S^1$ (vse, Fourier)
- $SU_2(\CC)$ (samo končno razsežne), Peter-Weyl na tem primeru (samo izrek, da smo s tem pokrili vse unitarne upodobitve), na tej točki zaključimo MSc predmet in cela teorija kompaktnih grup se lahko naredi za PhD predmet kot nadgradnja tega do sem.
