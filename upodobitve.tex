...
\[
    \langle \chi_{\pi}, \chi_{\11} \rangle = \frac{1}{4!} \left( 1 \cdot 4 + 6 \cdot 2 + 8 \cdot 1 \right) = 1,
\]
...
\todo{Skalarni produkt, unitarizabilnost, omeni Maschkeja nad $\CC$, o HS normi pri Plancherelovi formuli.}
...
\begin{zgled}
    ...
    \]
    Če torej uvedemo še skalarni produkt Fourierovih transformacij $\hat{f}$, $\hat{h}$ s predpisom
    \[
        \langle \hat{f}, \hat{h} \rangle = 
        \frac{1}{|G|^2} \sum_{\pi \in \Irr(G)} \deg(\pi) \cdot \tr \left( \hat{f}(\pi) \cdot \hat{h}(\pi) \right),
    \]
    lahko zadnjo enakost zapišemo kot
    \[
        \langle f, g \rangle = \langle \hat{f}, \hat{g} \rangle.
    \]
    Tej enakosti rečemo {\definicija Plancherelova formula}, interpetiramo pa jo z rekom, da Fourierova transformacija ohranja skalarni produkt.
    \todo{Nad $\CC$: zapiši s Hilbert-Schmidtovo normo.}
\end{zgled}
