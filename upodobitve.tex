...
Karakterji niso poljubne funkcije v $\hom(G,F)$, temveč vselej pripadajo prostoru $\hom_{\cl}(G,F)$ razrednih funkcij. Vemo že tudi, da so karakterji nerazcepnih upodobitev tudi linearno nedovisni. S pomočjo rezultatov o skalarnih produktih matričnih koeficientov bomo sedaj dokazali, da tvorijo celo \emph{bazo} prostora razrednih funkcij. 

\begin{trditev}
    Naj bo $G$ grupa in $F$ algebraično zaprto polje karakteristike tuje $|G|$. Tedaj karakterji nerazcepnih upodobitev tvorijo bazo prostora $\hom_{\cl}(G,F)$.
\end{trditev}

Zopet bomo za dokaz uporabili metodo povprečenja po grupi, a bomo to povprečenje še \emph{utežili}. Za dano upodobitev $\rho$ grupe $G$ in poljubno funkcijo $f \in \hom(G,F)$ definiramo {\definicija uteženo povprečje}\footnote{Kadar je $F = \CC$, uteženemu povprečenju rečemo tudi {\definicija nekomutativna Fourierova transformacija}. Njeno povezavo z običajno teorijo Fourierove transformacije bomo spoznali nekoliko kasneje.}
\[
    \EE_f(\rho) = \frac{1}{|G|} \sum_{g \in G} f(g) \rho(g).
\]
Za vsak $h \in G$ velja
\[
    \EE_f(\rho) \cdot \rho(h) = \frac{1}{|G|} \sum_{g \in G} f(g) \rho(gh)
    = \frac{1}{|G|} \sum_{g \in G} f(g) \rho(h) \rho(h^{-1}gh).
\]
Izpostavimo $\rho(h)$ in na grupi $G$ uporabimo notranji avtomorfizem $g \mapsto h g h^{-1}$, pa lahko zadnjo vsoto zapišemo kot
\[
    \rho(h) \frac{1}{|G|} \sum_{g \in G} f(hgh^{-1}) \rho(g).
\]
Kadar je $f$ celo razredna funkcija, je zadnja vsota ravno enaka $\rho(h) \cdot \EE_f(\rho)$. Uteženo povprečje je torej v tem primeru \emph{spletična} v $\hom_G(\rho, \rho)$. Če dodatno predpostavimo, da je $\rho$ nerazcepna upodobitev, po Schurovi lemi sklepamo, da je $\EE_f(\rho)$ skalarni večkratnik identitete. S tem zaključkom lahko dokažemo zadnjo trditev.

\begin{dokaz}
Naj bo $f \in \hom_{\cl}(G,F)$ razredna funkcija. Za vsako nerazcepno upodobitev $\pi$ grupe $G$ izberimo bazo in s tem matrične koeficiente $f_{i,j}^{\pi}$. Tvorimo razredno funkcijo
\[
    f \circ \inv \colon G \to F, \quad g \mapsto f(g^{-1})
\]
in z njo uteženo povprečje $\EE_{f \circ \inv}(\pi)$. Njeni matrični koeficienti so
\[
    \EE_{f \circ \inv}(\pi)_{i,j} = \frac{1}{|G|} \sum_{g \in G} f(g^{-1}) f_{i,j}^{\pi}(g) = \langle f, f_{i,j}^{\pi} \rangle.
\]
Ker je $\EE_{f \circ \inv}(\pi)$ skalarni večkratnik identitete, so torej vsi skalarni produkti $f$ z izvendiagonalnimi členi ($i \neq j)$ ničelni, skalarni produkti z diagonalnimi členi ($i = j$) pa so med sabo enaki. Funkcijo $f$ lahko zato razvijemo po matričnih koeficientih nerazcepnih upodobitev\footnote{Regularna upodobitev je vsota svojih izotipičnih komponent, te pa so ravno matrični koeficienti, zato lahko vsako funkcijo na $G$ razvijemo po matičnih koeficientih.} kot
\[
    f = \sum_{\pi \in \Irr(G)} \sum_i \alpha^{\pi} f_{i,i}^{\pi}
\]
za neke skalarje $\alpha^{\pi} \in F$.\footnote{Ti skalarji so po enakosti $\EE_{f \circ \inv}(\pi)_{i,i} = \langle f, f_{i,i}^{\pi} \rangle$ enaki $\alpha^{\pi} = \deg(\pi) \cdot \EE_{f \circ \inv}(\pi)_{i,i}$.} S tem smo funkcijo $f$ razvili po karakterjih $\chi_{\pi}$ s koeficienti $\alpha^{\pi}$. 
\end{dokaz}
...
\todo{Tule potrebujemo ortogonalnost karakterjev, zato tako pozno ...}

Dekompozicijo regularne upodobitve smo dobili iz matričnih koeficientov nerazcepnih upodobitev, torej gre za nekakšno \emph{notranjo} dekompozicijo. Obstaja pa tudi \emph{zunanja} dekompozicija, pri kateri iz regularne upodobitve same s pomočjo ustreznih projekcijskih spletičen (podobno kot pri dokazu Maschkejevega izreka) najdemo izotipične komponente regularne upodobitve. 

Za začetek si poglejmo, kako to deluje za projekcijo na $\11$-izotipično komponento. Splošen primer bomo razvili iz tega posebnega. Naj bo $G$ končna grupa z upodobitvijo $\rho$ na prostoru $V$ nad poljem karakteristike tuje $|G|$. Povprečna vrednost $\EE(\rho)$ je projekcijska spletična v $\hom_G(V,V)$ na $\11$-izotipično komponento v $V$, se pravi na podprostor fiksnih vektorjev $V^G$. 
