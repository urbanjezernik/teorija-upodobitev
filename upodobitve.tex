...
Po zadnjem izreku je za dano upodobitev $\rho$ in nerazcepno upodobitev $\pi$ število kompozicijskih faktorjev, ki so izomorfni $\pi$, neodvisno od kompozicijske vrste. Temu številu pravimo {\definicija večkratnost} $\pi$ v $\rho$ in ga označimo z $\mult_{\pi}(\rho)$. 
...
Oglejmo si tako situacijo še podrobneje. Naj bo $G$ grupa s polenostavno upodobitvijo $\rho = \bigoplus_{i \in I} \rho_i$ na prostoru $V = \bigoplus_{i \in I} V_i$, kjer je vsak $\rho_i$ nerazcepna podupodobitev. Naj bo $\pi$ neka nerazcepna upodobitev na prostoru $W$, ki se pojavi v $\rho$, se pravi $\mult_{\pi}(\rho) > 0$. Naj bo
\[
    \Izotip_{\pi}(\rho) = \bigoplus_{i \in I \colon \ \rho_i \cong \pi} V_i
\]
zbor vseh podprostorov, na katerih $G$ deluje kot s $\pi$. Ta prostor imenujemo {\definicija $\pi$-izotipična komponenta} upodobitve $\rho$.  Po konstrukciji je ta sicer odvisna od izbrane dekompozicije upodobitve $\rho$, a premislimo, da je ta odvisnost le navidezna.

\begin{trditev}
Naj bo $G$ grupa s polenostavno upodobitvijo $\rho$. Za vsako nerazcepno upodobitev $\pi$ grupe $G$ je $\pi$-izotipična komponenta $\Izotip_{\pi}(\rho)$ neodvisna od izbire dekompozicije $\rho$, ki opazi njeno polenostavnost. 
\end{trditev}
\begin{dokaz}
Premislimo, da je $\Izotip_{\pi}(\rho)$ enaka vsoti (ne nujno direktni) vseh $G$-invariantih podprostorov prostora $V$ upodobitve $\rho$, ki so kot upodobitev izomorfni $\pi$. Ta opis $\pi$-izotipične komponente je neodvisen od izbire polenostavne dekompozicije $\rho$.

Naj bo $W$ omenjena vsota podprostorov, ki so kot upodobitev izomorfni $\pi$. Seveda je $\Izotip_{\pi}(\rho) \leq W$. Dokažimo, da velja tudi obratna neenakost. Naj bo $U$ direktna vsota tistih prostorov $V_i$, ki kot upodobitev \emph{niso} izomorfni $\pi$. Velja $V = \Izotip_{\pi}(\rho) \oplus U$. Opazujmo projekcijo $p \colon V \to U$ z jedrom $\Izotip_{\pi}(\rho)$. Naj bo $Z \leq W$ podprostor, ki je kot upodobitev izomorfen $\pi$. Zožitev $p|_Z$ je spletična v $\hom_G(Z, U)$, ki je po Schurovi lemi ničeln prostor. Torej je $p(Z) = 0$ in s tem $Z \leq \Izotip_{\pi}(\rho)$. Ker je bil $Z$ poljuben, smo s tem dokazali $W \leq \Izotip_{\pi}(\rho)$.
\end{dokaz}

\begin{domacanaloga}
Naj bo $G$ grupa s polenostavno upodobitvijo $\rho$ na prostoru $V$. Premisli, da je $V$ direktna vsota izotipičnih komponent, ki pripadajo paroma neizomorfnim nerazcepnim upodobitvam.
\end{domacanaloga}
...
\item Naj bo $G$ grupa z upodobitvijo $\rho$ na prostoru $V$ in naj bo $\pi$ neka njena enorazsežna upodobitev. Taka upodobitev je seveda nerazcepna. Vektor $v \in V$ pripada izotipični komponenti $\Izotip_{\pi}(\rho)$, če in samo če grupa $G$ na prostoru $\langle v \rangle$ deluje kot s $\pi$, se pravi
\[
    \Izotip_{\pi}(\rho) = \left\{ v \in V \mid \forall g \in G \colon \ \rho(g) \cdot v = \pi(g) v \right\}.
\]
...
Do zdaj smo upodobitve obravnavali z bolj konceptualnega stališča. 
...
\section{Polenostavnost in regularna upodobitev}
\section{Karakterji}
\section{Konkretni primeri}

\subsection{Abelove grupe}
\subsection{Razširjeni zgled $\GL_2(\field_p)$}
\subsection{Razširjeni zgled $S_n$}
...
\section{Liejeve grupe in mreže}

\subsection{Razširjeni zgled $\SL_2(\CC)$}
\subsection{Mreža $\SL_2(\ZZ)$}
