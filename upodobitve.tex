...
\[
    \langle f, h \rangle = \frac{1}{|G|^2} \sum_{\pi \in \Irr(G)} \chi_{\pi}(1) \tr(\hat{f}(\pi) \cdot \hat{g}(\pi)).
\]
Če torej uvedemo še skalarni produkt Fourierovih transformacij $\hat{f}$, $\hat{g}$ s predpisom
\[
...
\]
Tej enakosti rečem {\definicija Plancherelova formula}, interpetiramo pa jo z rekom, da Fourierova transformacija ohranja skalarni produkt.
\end{zgled}
