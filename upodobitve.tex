...
Preden dokažemo izrek, pojasnimo, kako in zakaj nam prideta prav končnost grupe $G$ in ustrezna karakteristika polja $F$. Ti dve predpostavki namreč odpirata vrata orodju {\definicija povprečenja po grupi}. Za dano funkcijo $f \in \hom(G,F)$ lahko v tej ugodni situaciji izračunamo njeno povprečno vrednost
\[
    \EE_G(f) = \frac{1}{|G|} \sum_{g \in G} f(g) \in F.
\]
...
\[
    \EE_G(\rho) = \frac{1}{|G|} \sum_{g \in G} \rho(g) \in \hom(V,V).
\]
Ni težko preveriti, da je $\EE_G(\rho)$ projekcijska spletična v $\hom_G(V,V)$ na podprostor fiksnih vektorjev $V^G$. 
...
\[
    ...
    \frac{1}{|G|} \sum_{g \in G} f_{i,j}(g^{-1}) g \cdot f_l.
\]
Na zadnjem uporabimo še $f_k^*$, pa dobimo
\[
    0 = \frac{1}{|G|} \sum_{g \in G} f_{i,j}(g^{-1}) \langle f_k^*, g \cdot f_l \rangle =
    \frac{1}{|G|} \sum_{g \in G} f_{i,j}(g^{-1}) f_{k,l}(g),
\]
kar je enakovredno $\langle f_{i,j}, f_{k,l} \rangle = 0$, se pravi ortogonalnosti matričnih koeficientov.
\end{dokaz}

Na soroden način lahko analiziramo skalarne produkte znotraj matričnih koeficientov ene same nerazcepne upodobitve.

\begin{trditev}
Naj bo $G$ končna grupa z nerazcepno upodobitvijo $\pi$ nad algebraično zaprtim poljem karakteristike tuje $|G|$. Tedaj za vse $1 \leq i,j,k,l \leq \deg(\pi)$ velja
\[
    \langle f_{i,j}, f_{k,l} \rangle = \frac{1_{i = l, j = k}}{\deg(\pi)}.
\]
\end{trditev}
\begin{dokaz}
    \todo{}
...
Iz rezultatov zadnjega razdelka sledi, da je nad algebraično zaprtim poljem ničelne karakteristike (na primer zelo ugodnim poljem $\CC$) kategorija upodobitev dane končne grupe popolnoma določena z nerazcepnimi upodobitvami, ki jih lahko razumemo s pomočjo karakterjev. Vsak karakter nerazcepne upodobitve je razredna funkcija, te pa so enolično določene s svojimi vrednostmi v predstavnikih konjugiranostnih razredov. Če število konjugiranostnih razredov označimo s $\kk(G)$, velja torej
\[
    |\Irr(G)| \leq \kk(G).
\]
S podrobnejšim študijem karakterjev bomo v tem razdelku (med drugim) dokazali, da je zadnja neenakost v resnici \emph{enakost} in da lahko torej samo število nerazcepnih upodobitev izračunamo neposredno iz algebraične strukture grupe.
...
\todo[inline]{
    - ortonormalnost
    - Vključi 4.3.3. Decomposition of class functions in dokaz o številu nerazcepnih upodobitev
    - zgled: $S_4$ iz Fulton-Harris
    - večkratnosti 4.3.5
    - nerazcepnost
    }
...
\todo[inline]{
- vključi kakšen preprost zgled; omeni, da gre za to, da ni treba razstavljati regularne upodobitve; ta zgled lahko potem kasneje razširimo z \GAP.
}
