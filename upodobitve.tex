...
\begin{itemize}
    ...
    \item Naj bo $f \in \fun(G,F)$ funkcija na $G$ in $\rho_{\fun}$ regularna upodobitev grupe $G$. Vrednost $\hat{f}(\rho_{\fun})$ je linearni endomorfizem prostora $\fun(G,F)$. Pri tem se karakteristična funkcija $1_x$ za $x \in G$ preslika v
\[
...
\end{trditev}
\begin{dokaz}
Velja $\rho(g^{|G|}) = \rho(1) = \id$, zato je $\rho(g)$ linearna preslikava končnega reda. Take preslikave so diagonalizabilne.\footnote{Diagonalizabilnost sledi iz obravnave Jordanove normalne forme preslikave $\rho(g)$.} V posebnem je zato vsaka lastna vrednost $\lambda \in \Eig(\rho(g))$ končnega reda v $\CC^*$. S tem je seveda
\[
    \chi_{\rho}(g) = \sum_{\lambda \in \Eig(\rho(g))} \lambda \in \bar{\ZZ},
    \quad
    |\chi_{\rho}(g)| \leq \sum_{\lambda \in \Eig(\rho(g))} |\lambda| = \deg(\rho)
\]
in hkrati
\[
    \chi_{\rho}(g^{-1}) = \sum_{\lambda \in \Eig(\rho(g))} \lambda^{-1}
    = \sum_{\lambda \in \Eig(\rho(g))} \overline{\lambda}
    = \overline{\chi_{\rho}(g)}.
\]
\end{dokaz}
...
Dokaz bomo navezali na edino mesto, kjer smo že videli ulomek $|G|/\deg(\pi)$, in sicer je to lema o Fourierovi transformaciji razredne funkcije. Ko funkcija, vzdolž katere izvedemo transformacijo, slika v kolobar algebraičnih celih števil, lahko lemo o Fourierovi transformaciji razredne funkcije zaostrimo na naslednji način.
    ...
    \[
        \frac{|G|}{\deg(\pi)} \cdot [ \chi_{\pi}, \chi_{\pi} ] = \frac{|G|}{\deg(\pi)} \in \bar{\ZZ} \cap \QQ = \ZZ.
    \]
    \end{dokaz}

\subsubsection{Skalarni produkt}

Polje $\CC$ je opremljeno s standardnim skalarnim produktom $\langle z, w \rangle = z \cdot \overline{w}$. Ta produkt lahko razširimo na vsak končnorazsežen kompleksen vektorski prostor. Obravnavali bomo dve taki razširitvi, in sicer na prostor funkcij $\fun(G,\CC)$ ter na prostor, na katerem upodabljamo grupo $G$.

Opazujmo najprej prostor funkcij $\fun(G,\CC)$. Vemo že, da ga lahko opremimo s skalarnim produktom $[\cdot, \cdot]$. Ker pa je ta prostor kompleksen, lahko nanj vpeljemo še nekoliko drugačen skalarni produkt, ki je standarden v kompleksnih prostorih, in sicer
\[
...
\subsubsection{Parsevalov izrek}

Fourierova inverzija je nekoliko asimetrična. To lahko popravimo tako, da jo uteženo povprečimo z neko drugo funkcijo $h \in \fun(G,F)$. Dobimo
\[
    \sum_{g \in G} f(g) h(g^{-1}) = \frac{1}{|G|} \sum_{\pi \in \Irr(G)} \sum_{g \in G} h(g^{-1}) \chi_{\pi}(1) \tr \left( \hat{f}(\pi) \cdot \pi(g) \right),
\]
kar lahko z enakim argumentom kot prej zapišemo kot
\[
    [ f, h ] = \frac{1}{|G|^2} \sum_{\pi \in \Irr(G)} \chi_{\pi}(1) \tr(\hat{f}(\pi) \cdot \hat{h}(\pi)).
\]
Tej enakosti rečemo {\definicija Parsevalov izrek}.

    \todo{Kompleksna števila imajo konjugiranje, tudi prostore nad $\CC$ opremimo s skalarnim produktom, ki je wlog $G$-invarianten po povprečenju.}

\todo{En drug, standarden Skalarni produkt, unitarizabilnost, omeni Maschkeja nad $\CC$, o HS normi pri Plancherelovi formuli.}
...
\todo[inline]{??? čisto nekomutativen problem: ga izpeljemo spet iz konvolucije identifikatorjev konjugiranostnih razredov, komutatorji; Frobeniusova formula, omeni LOST Ore domneva}
