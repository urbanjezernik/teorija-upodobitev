...
Avstralski matematik Geordie Williamson je na mednarodnem matematičnem kongresu leta 2018 opisal teorijo upodobitev na naslednji način.
...
\end{align*}
Grupa $B_p$ je {\definicija Borelova podgrupa}, grupa $U_p$ pa {\definicija unipotentna podgrupa}. Seveda je $B_p = D_p \times U_p$. Grupa $G_p$ ima tako vrsto podgrup
\[
...
\[
    \Pi \colon G_p \to S_{p+1} = \Sym(\PP^1(\FF_p)),
\]
...
\[
    \SL_2(\FF_p) = \left\{ \begin{pmatrix} a & b \\ c & d \end{pmatrix} \mid a,b,c,d \in \FF_p, \ ad - bc = 1 \right\}.
\]
...
\[
    ...
    \end{pmatrix}
\]
Ker lahko vsako matriko v $\SL_2(\FF_p)$ z vrstičnimi operacijami pripeljemo do identitete, sklemeno, da elementa $S_+$, $S_-$ \emph{generirata} grupo $\SL_2(\FF_p)$. 
...
\[
    ...
    = S_+
\]
in sklenemo $S_+ \in [G_p, G_p]$. Sorodno dobimo $S_- \in [G_p, G_p]$. Ker $S_+,S_-$ generirata $\SL_2(\FF_p)$, dobimo še drugo neenakost.
\end{dokaz} 
...
\subsubsection{Konjugiranostni razredi}
...
\begin{enumerate}
    ...
    \[
        ...
        \end{pmatrix}. 
    \]
    Centralizator vsakega takega elementa je generiran z njegovimi potencami in skalarji, zato je moči $(p-1)p$. Konjugiranostni razred je torej velikosti $p^2 - 1$. Vseh takih razredov je $p-1$.
    ...
    \]
    Konjugiranostni razred je torej velikosti $p(p-1)$. Vseh takih razredov je $p (p-1)/2$.\footnote{Če zamenjamo v zgornji matriki $b$ z $-b$, dobimo podobno matriko.}
\end{enumerate}
...
\end{tabular}
\caption{Konjugiranostni razredi v $G_p$: njihov tip, velikost in število razredov določenega tipa}
\end{table}
...
Preden nadaljujemo z iskanjem nerazcepnih upodobitev grupe $G_p$, se še enkrat ozrimo na klasifikacijo konjugiranostnih razredov. Tekom določanja velikosti razredov smo naleteli na dva posebna centralizatorja polenostavnih elementov, in sicer $T_r$ in $T_{nr}$. Ta dva centralizatorja bosta igrala pomembno vlogo v teoriji upodobitev grupe $G_p$. Prvemu pravimo {\definicija razcepni torus}, drugemu pa {\definicija nerazcepni torus}. Za razcepni torus velja
\[
...
\subsubsection{Tabela karakterjev}
